...
func TestCsvColumnToSlice(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "FailedRead", col: 1, exp: nil, expErr: iotest.ErrTimeout, r: iotest.TimeoutReader(bytes.NewReader([]byte{0}))},
		{name: "FailedNotNumber", col: 1, exp: nil, expErr: ErrNotNumber, r: bytes.NewBufferString(csvData)},
		{name: "FailedInvalidColumn", col: 4, exp: nil, expErr: ErrInvalidColumn, r: bytes.NewBuffer([]byte(csvData))},
	}

	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			res, err := csvColumnToSlice(tc.r, tc.col)
