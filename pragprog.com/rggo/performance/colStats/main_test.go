...
func TestRun(t *testing.T) {
	...
	testCases := []struct {
		...
		{name: "RunFailRead", col: 2, op: "avg", exp: "", files: []string{"./testdata/example.csv", "./testdate/fakefile.csv"}, expErr: os.ErrNotExist},
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.expErr != nil {
				...
				if !errors.Is(err, tc.expErr) {
					t.Errorf("Expected error %q, got %q instead", tc.exp, err)
				}
				return
			}
